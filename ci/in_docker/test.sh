...
for PYVER in ${PYTHONVERS} ; do
  ...
  PYTEST_FAIL="NO"
  if ! "python${PYVER}" -m pytest --cov-config=.coveragerc --cov-fail-under=100 "--cov=${MAIN_MODULE}" --cov-report=xml:test-cov.xml --cov-report=html --cov-report=term-missing ; then
    PYTEST_FAIL="YES"
  fi
